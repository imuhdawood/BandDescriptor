...
def perform_clustering_and_generate_plots(data, features_col, cell_type,celltypes_selected=None):
    ...
    n_components=20,  # Start with a larger number of components
    ...
    mean_precision_prior=1.0,  # Moderate prior on mean precision
    ...
    n_init=1,  # Multiple initializations
...
if __name__ == "__main__":
    ...
    #Reading Consolidated File containing cell type info
    CONSOL_FILE = f'{DATA_DIR}/{EXP_TAG}.csv'
    consolDf = pd.read_csv(CONSOL_FILE, index_col='Unnamed: 0')

    celltypes_selected = consolDf[ANNOT_LEVEL].tolist()
    ...
    for cell_type in celltypes_of_interest:
        ...
        copy('/well/rittscher/users/qwi813/CellMiroEnviron/application/mine/band_features_clusters_tweaking_subtype.py',f'{OUTPATH}/br{radii}_{TAG}/band_features_clusters.py')
